...
x, y = np.loadtxt("./data/paper_multimodal_data.txt")
x_val1, y_val1 = np.loadtxt("./data/paper_multimodal_data_test.txt")

device = torch.device("cpu")

x_train = np.transpose(np.vstack((x, np.random.randn(len(x)))))
x_eval = np.transpose(np.vstack((x_val1, np.random.randn(len(x_val1)))))
...
#torch.save(net.state_dict(), "./weights/multimodal_3layer_epochs_{}_withdropout.pt".format(epochs))
...
#plt.savefig("./plots/loss_function_3layer_withdropout_multimodal.png")
#plt.show()
...
#for j in range(4):
    ...
    #val_data, val_labels = np.loadtxt("./data/paper_multimodal_data_test_{}.txt".format(j))
...
#plt.savefig("./plots/multimodal_prediction_full_no_epochs_{}_3layer_withdropout.png".format(epochs))
#plt.show()
#plt.close()
#fig, axs = plt.subplots(nrows=2, ncols=2, sharex=True, sharey=True)
#axs = axs.ravel()
...
#for j in range(4):
    #data_pred = []
    #help_arr = np.zeros((1,2))
    #val_data, val_labels = np.loadtxt("./data/paper_multimodal_data_test_sparse_{}.txt".format(j))
    #for data in val_data:
        #help_arr = np.append(help_arr, latent_variables(data), 0)
    #help_arr = np.delete(help_arr, 0, 0)
    #val_data_torch = torch.from_numpy(help_arr)
...
#plt.savefig("./plots/multimodal_prediction_sparse_no_epochs_{}_3layer_withdropout.png".format(epochs))
#plt.show()
#plt.close()
#plt.plot(x_val1, y_val1, '.r', label = "Real Data")
#plt.legend()
#plt.savefig("./plots/multimodal_real.png")
#plt.show()
